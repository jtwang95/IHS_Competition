...
fit.gam = gam(
  ...
  data = tmp %>% filter(CompType != "steps") %>% filter(!is.na(PreSleep)),method="REML")
summary(fit.gam)
...
fit.gam = gam(
  ...
  data = tmp %>% filter(!is.na(PreReportStep)),method="REML")
summary(fit.gam)
...
fit.gam = gam(
  ...
  data = tmp %>% filter(!is.na(PreReportSleep)),method="REML")
summary(fit.gam)
...
fit.gam = gam(
  ...
  data = tmp %>% filter(!is.na(PreReportMood)),method="REML")
summary(fit.gam)
